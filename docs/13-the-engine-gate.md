...
`games/warren` is **0 / 35** box-only meshes and **0 / 16** on *all three* texture slots — no material in
that game leaves one empty. Neither number can move again, and review 24 pointed out that a metric which
...
|---|---|---|---|
| **R1** | **Intra-mesh repeat** — every `repeat` block in every `.mesh`, by `count` and `step` | no block with `count > 6` at a constant step | `duckboards.mesh` at `count 7`; one over the line, and the only one |
| **R2** | **Placement repeat** — every prefab instanced more than three times, by *N* and its number of distinct **override sets** | nothing placed more than six times has fewer than three distinct sets | not yet counted |
