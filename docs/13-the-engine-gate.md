...
| **F2** | 38 | **The warden's form.** Open across six passes. `warden_figure.mesh` was **nine `solid Box` parts, every one axis-aligned and none rotated** — a stack of rectangles. It is the antagonist of a horror game and it read as a crate. `Part` already carries `rotation` and `Solid` already ships `Cylinder` with a `top_radius`, so a flared coat is authorable in text with no Rust | A capture from `at_warden.snapshot`. **(a) Silhouette:** sample the figure's left and right silhouette x at **20 evenly spaced heights** between the top of the brim and the hem (edge = first crossing of the midpoint between the figure's median luma and the background's, stated in the report). The left profile takes **≥ 8 distinct x values**, no single value repeats for more than **3 consecutive samples**, and the profile **widens monotonically over the lowest 25% of the height by ≥ 8% of maximum width** — a hem that flares. **(b) Asymmetry:** mean \|left width − right width\| over the 20 samples ≥ **6% of maximum width**. **(c) Form, not side-lighting:** on a **vertical** profile down the coat's lit half at a named column, mean adjacent \|ΔL\| ≥ **3.0** over ≥ 60 px, non-monotonic — folds, not a gradient | 🟡 **built (s26), awaiting verdict — and three of the five sub-clauses do not discriminate, measured against the old model as a control.** Built: fourteen parts, truncated cones from a 0.45 m hem to a 0.115 m hat crown, faceted, a storm collar the head sits inside so no face shows under the brim, and a lean. **(a) part 2 is the clause that works** — old model **5 consecutive samples at x=1048**, new model **max run 2**, so it fails the old and passes the new exactly as designed. **(a) part 1 does not**: old 14 distinct values, new 16 — both pass. **(c) does not**: the old nine-box model measures **3.92** against the new model's **3.44** at the same column, so the *worse* model scores higher and the clause would have been closed on the defect. **(a) part 3 and (b) are unmeasurable on this frame**, with evidence: the figure is **darker than its background at the brim** (luma **8** against **42**) and brighter at the hem, so the stated one-sided edge criterion cannot find this silhouette at all; and the warden's own lamp plus the bunk behind it run continuously bright to its right, so the right profile has no measurable edge. Clause (b) needs an instrument that does not assume figure > background, and (c) needs one that is not satisfied by hard box edges |
| **F2b** | **new, r28** | **The warden respects the level — the largest gap in the plan, and it was not in it.** `move_the_warden` writes `Transform::translation` straight toward the player with no collider and no sweep; `watch_for_you` sets `sees_you` from `distance(...) <= WARDEN_SIGHT` alone, with no line of sight. **The antagonist sees through cast-iron bulkheads and walks through them.** Its own doc comment says *"the room is open enough that it does not read as broken"* — **that was written for `scenes/warren.scene`, and the shipped level is `scenes/generated.scene`**, fourteen sections divided by bulkheads. **F6 exists to stop a warden being as loud through a wall as through a doorway; without this, F6 removes the lie and the omission puts it straight back in the same thirty seconds.** Not item 18 and not pathfinding: `cast_shape` for sight and `move_shape` for motion, both built, both already used in this game | A headless test placing the warden across a bore wall from the player, in `pursue`: after 300 ticks **(a)** `Facts::get("sees_you")` has been `false` throughout, and **(b)** the warden has never been within 0.30 m of a static collider face and has never crossed a wall plane — with a second case through an open cross-passage in which it **does** reach the player. Plus one capture from `at_warden.snapshot` after 120 ticks showing the figure **against**, not inside, the bulkhead | ⬜ |
