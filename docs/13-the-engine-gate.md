...
re-scoped the gate in session 26 and the order is now **F2, F4, F5, F1, F3, F6**, six rows, with
everything else cut or moved to the editor or to the first published game. **Read §1b before §2.**
...
- **A row per object.** Review 25 ruled that *"everything else is a specific object, not a property of
  ...
  their own number and their own review. **Rows 33, 34, 36, 37, 30 and the call plate are merged into
  one row, F5**, closed by one pass over the objects a player actually stops in front of.
...
**FINISH** — the demo game's remaining work, and the whole of it. Six rows.
...
| Bucket | Rows | Count |
|---|---|---|
| **FINISH** | **F1** (15 + 35, the opening) · **F2** (38, the warden) · **F3** (32, wayfinding) · **F4** (12b, the torch beam) · **F5** (33 + 34 + 36 + 37 + 30 + the call plate, merged) · **F6** (26, audio occlusion) | **6** |
| **EDITOR** | 20 (the protocol's write half — a real M4 prerequisite) · 17 (`MeshInstance` allocations — editor responsiveness) · 3c (keep current) | 3 |
| **GAME 2** | 27 (tilemap + isometric sort) · 18 (`mod-pathfinding`) · 28 (skinning, Q41) · 12 (particles) · 29 (decals) · 11b (alpha cutout) · 19 (widen the performance evidence) · 16 (the crowd-agent ADR) · 21's screen-space half · 23 (normal-map mips) | 10 |
| **CUT** | 39 (`games/scarp`) · 14 · 12c · 12f part two · 13b · 13's remainder · 22 · 9b | 8 |
...
| **12f** part two, the sky's picture | Frozen game. |
| **13b** bloom and fog authored at values that do nothing | Reopened against `games/atrium`. The Warren's own bloom-threshold defect was found and fixed in s24, and **F4 re-measures fog in the Warren** because a volumetric beam raymarches through it — so the only part of this row with a player in it is covered. |
| **13** the remainder of the texture generator | `amadeo-texture` shipped, its anti-lattice clause passed on r13, and the Warren's sixteen materials are generated by it. The remainder is polish on a tool that already has a user. |
| **22** shadow-edge dithering | Measured on `games/atrium`'s sun band. The Warren has no sun. |
| **9b** one-field tuple variants in `amadeo-derive` | Already demoted. An authoring convenience. |

Two rows move to **GAME 2** rather than being cut, because the case that breaks them is that game's
normal case and not this one's:
...
Each is one session-sized piece of work with **one capture that closes it**. Conditions are the
existing rows' conditions where those were already falsifiable.

| # | Was | Item | Closed when | Status |
|---|---|---|---|---|
| **F1** | 15 + 35 | **The opening — the first frame you see and the first thing you do.** Two rows, and they are the same thirty seconds. The title screen is unchanged since review 2 and is the only frame in the game with no backdrop: title left edge x=502, focus bar x=496, `BEGIN` x≈505, `CONTINUE` x=504 — four left edges inside 9 px — and the label shifts 1 px as the highlight moves. Then you wake up and your first input is the mouse, and **yaw 270 from the spawn is the worst frame in the game**: mean 109.7, the brightest of nine, the camera near-clipping a lining wall with two thirds of the frame an incoherent tangle of plate edges. Review 27: *"the fix is a spawn position, not geometry"* | The title composes over **a rendered frame of the tunnel** rather than a fill; its four left edges are either deliberately one column (within 1 px) or separated by ≥ 24 px; the label's left edge does not move when the highlight does; and **no yaw at the spawn near-clips geometry** — the four cardinal yaws all read mean luma within 25 levels of each other with no surface closer than 0.8 m | ⬜ |
| **F2** | 38 | **The warden.** Open across six passes. Four axis-aligned boxes at 5×, one flat pale grey-green, nothing on it varying more than ~15 levels across its whole front face — outline without form. It is the antagonist of a horror game and it reads as a crate | A capture at `at_warden` shows **no axis-aligned silhouette edge longer than 25% of the figure's height**, and the figure's front varies by **≥ 40 levels** between its lit and unlit sides | ⬜ |
| **F3** | 32 | **Wayfinding.** The section letters are `manhattan % 5` — a distance ring wearing a name. `docs/11` §5.4 states three requirements and this meets one. Five letter meshes exist (`H I M O T`) | A section sign **names a place and points at one**: letters spell a name from an authored alphabet, and the sign carries a direction a player can act on. Two captures at two different signs, showing two different names | ⬜ |
| **F4** | 12b | **The torch beam in the air.** `docs/05` M3 exit gate 5 names it the biggest remaining visual step, and ADR 0073 records that it raymarches through exactly the fog that landed. *"Most of what a horror flashlight is"* | A capture with the beam on and one with it off differ by **≥ 25 levels of mean luma inside the cone and < 3 outside it**, and an `Environment` that does not ask for it is byte-identical | ⬜ |
| **F5** | 33 34 36 37 30 + call plate | **The objects you stop in front of**, merged from six rows: the grime blur (mean adjacent \|ΔL\| **1.55** against a 3.0 bar), the crates brighter than the tunnel (**117.3** against **71.2**), the key board's broken orange edge (12–15 px on rows 520/560/600), the section sign's edge-on gold out-orangeing `accent` itself, the key's plinth 2.9 m from the key it holds, and 52 × 39 px of unmodelled orange call plate | One capture per landmark snapshot, **all passing at once**: no surface at mean adjacent \|ΔL\| < 3.0 at 3 m, nothing in the shelter brighter than the lining behind it, `accent` the most saturated orange in every frame it appears in, and every `Interactable` marker unbroken across its own edge | ⬜ |
| **F6** | 26 | **Audio occlusion.** `docs/11` §9 makes it a gameplay requirement rather than polish: *"a warden exactly as loud through a wall as through a doorway makes the whole mechanic a lie"*. The game is made of corridors and its antagonist is found by ear | A voice behind a wall is measurably attenuated against the same voice at the same distance through an opening, asserted **headlessly in `VoiceTracker`'s terms** rather than by listening | ⬜ |

**Order: F2, F4, F5, F1, F3, F6.** The warden first because it is the antagonist and six passes have
not moved it. The beam second because it changes every frame in the game at once. F5 third because it
is one pass over six objects. Then the title screen, wayfinding, and audio.
