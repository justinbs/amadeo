...
| # | R# | Item | Closed when | Status |
|---|---|---|---|---|
| 12a | — | **Make the Atrium a room rather than a showroom**, before 12c. Four things, none of them fiction: an **opening** (the single highest-value change -- it turns uniform light into light with a direction and a source, and an atrium is canonically a space with a roof opening); a **doorway for human scale**, without which texel density is unjudgeable because nobody can tell whether a 2 m slab is 2 m; **somewhere to look into**, which is what makes depth and fog legible and gives the eye a reason to travel rather than enumerate; and **adjacency instead of display** -- the lamp by the table, the screen partitioning something, the generator in a corner, which costs only changed translations | A capture reads as a place rather than a list: light arrives through an opening, a door gives the eye a scale reference, and no prop stands where only a demonstration would put it | ✅ **done** (s21) — passed on review 8 |
| 12c | — | **A sky for `games/atrium`.** See the box above: `sky ""` blocked four things at once, and the two existing `.hdr` generators are the precedent | The Atrium names a real environment map, and the lamp, the glass and the wall gradient each read as themselves in a capture | 🟡 **built; held open behind 12f** (s21) — the map is real and ADR 0079 split its two jobs. Rebalanced to `SCALE 0.5` / `sky_ambient 0.68`, same product, so the fill did not move by a level (framing stone reads 80,81,85 at both) while the aperture went from a 15-level colour spread to 29. Pushing further to 0.35/0.971 buys 4 more levels of chroma for 26 of brightness and was rejected. Review 10 declined to close it because the glass — one of the four symptoms `sky ""` was named as unblocking — is still present. Now item 12f |
| 12d | — | **The Atrium was 20 × 20 × 4 m with the camera at y = 3.69, thirty-one centimetres below the roof**, so every upward view was flat grey and the oculus was invisible from anywhere a player could stand: at 25° the view ray met the roof at z = 5.0, before the opening at z = 4.0. The look-up view review 9 called the best image this project has produced was taken in a room nobody plays in | A capture from the player's own camera, with no scene edit, shows the oculus and the light coming through it | ✅ **done** (s21) — passed on review 10, which called the result "the best frame this project has produced" and singled out the warm-sunlit-against-cool-sky-lit floor as the first time the lighting reads as daylight |
| 12e | — | **`amadeo capture --pitch/--yaw`** (review 10 item 3, and ADR 0079's sibling). A capture could only ever show the authored camera, and this project judges nearly everything by capture — so checking a ceiling meant editing the game, capturing and reverting, which was done repeatedly and twice left an edit behind | A reviewer can look anywhere in a game without modifying it, and the cameras are provably put back | ✅ **done** (s21) — passed on review 10, which used it seven times and called it the most useful thing built for its job since capture itself. It found 12d, 12g, and the mis-sampling behind 12f |
| 12f | — | **The glass does not read as glass** (review 10 item 3). **Not** the cause proposed: `draw_run` is one closure called for both pipelines, so the blended pass binds the same environment at slot 3, and setting the pane `metallic 1.0` moves it from (204,209,205) to (123,133,138) — the specular path demonstrably works. The review's two probe points, (512,260) and (470,280), are both **off the pane**, which occupies only y = 288–306, x = 506–536 in the default view (~30 × 20 px) and reads 187 there against a 112 wall. The real cause is one level deeper: **the environment map is a featureless gradient, so there is nothing to reflect.** `daylight.rs` deliberately has no sun disc, to stop the sun being counted twice in the irradiance. A mirror aimed at a smooth gradient returns a flat tint, which is exactly what the pane shows. The fix is ADR 0079's shape one level down — **split the map's content, not just its intensity**: structure and a sun disc in the specular chain, excluded from the irradiance, which is what keeping an analytic light out of the ambient probe means in every comparable engine | The pane shows a specular sheet or a Fresnel rim that moves with the camera, and is distinguishable from a hole in a frame | 🟡 **part one done (ADR 0080); part two open** (s21) — the cause was neither of the two proposed. `ALPHA_BLENDING` multiplies the WHOLE shader output by coverage, so at alpha 0.34 a highlight was bounded above by `0.34·S + 0.66·W` and was arithmetically impossible rather than dim. Premultiplied output fixes it, opaque byte-identical, and the Atrium's pane went from **1 level** brighter than the wall beside it to **10**. Part two — structure and a sun disc in the specular chain, excluded from the irradiance — still open, and its close condition needs **azimuthal** content: `daylight.rs` is a function of elevation alone, so a reflection in it cannot move when the camera yaws |
| 12g | — | **Three of four quadrants are empty** (review 10 item 4). Every prop is in the north-west quadrant and `--yaw 180` is 1280 × 720 of a single value. Item 12a passed on review 8 against the authored view only; 12e is what made this visible. Needs a counterpart to the doorway on the south wall and one occluder breaking each 20 m sightline — Amnesia and SOMA both refuse straight runs beyond 10–15 m | No capture at yaw 0/90/180/270 is a single flat value | ✅ **done** (s21) — the fix was not more props: the camera sits at y ≈ 4.35 in an 8 m room, so anything under 2 m never enters frame. Four gallery runs at 3.7–4.9 m, a stair up to the west one, a screen wall and a ledge. The player start moved to z = −1 so the camera is not jammed against the south wall, which is what made three of the four views unfixable |
| 12h | — | **Normal-offset shadow bias.** `6ddf9ac` cut the roof-to-wall light leak from a peak of 143 to 110 by lowering the bias and tightening the shadow box, but a three-pixel residual remains and going lower invites acne. The real fix offsets the shadow lookup along the surface **normal** rather than along depth, which removes contact leaks without the trade. Engine work, not a scene number. **Correction to `6ddf9ac`'s commit message, found by checking my own claim against the source afterwards:** that commit said a larger shadow box turns the same authored bias into a larger world-space offset. It does not. `fit_cascade` sets `bias = shadow_bias / (far - near)` with `far = 2 × radius`, so the world-space offset is `shadow_bias` **whatever the box size** — the normalisation CLAUDE.md describes is real and it works. What a larger box actually costs is **texel footprint**: one `shadow_resolution` spread over more world, ~21 mm per texel at radius 22 against ~29 mm at 30, and contact leak scales with that. So the leak was most likely present all along at `0.03` and simply had no viewable junction until the room grew, rather than being a regression this session introduced. The bias was always too high | The Atrium's wall-to-ceiling junction reads within a level or two of the shadowed wall, with no acne on the floor | 🟡 **normal-offset landed (ADR 0081); rescoped** (s21) — junction peak 115 → 109, corner 111 → 106, no acne at four checked points. What remains is **not a bias artifact**: doubling `shadow_resolution` narrows the band from 3 px to 2 while the peak holds at 114, so its width scales with texel size. It is the topmost row of fragments sitting within one texel of the occluder, where 2–3 of the 9 PCF taps escape. The fix is contact hardening — a kernel problem |
| 13a | — | **Normal and metallic-roughness maps for the Atrium's stone**, so the other two texture paths get a user. ADR 0047 and ADR 0048 are both still exercised by zero content, which is two thirds of session 20's original finding. Cheap from here: `surfaces.rs` already computes a joint distance field, so a height-to-normal map is a Sobel over the same function, and a metallic-roughness map for stone is close to a constant green channel. **Q31's trap applies** -- both are data rather than colour, so their sidecars need `color_space = "linear"` and nothing warns if they forget | `games/atrium`'s stone samples all three maps, and a capture shows the joints recessing under a raking light rather than being painted on | ✅ **done** (s21) — widened to `slate` as review 11 asked, since the walls are the largest surface in the game. Three maps per stone from **one height field**, so colour, normal and roughness cannot drift apart. Per-slab tone landed in the same pass, hashed per slab rather than sampled from noise so it is flat across a slab and discontinuous at the joint. `normal_texture` and `metallic_roughness_texture` both go 15/15 empty → 11/14 |
...
| # | R# | Item | Closed when | Status |
|---|---|---|---|---|
| 13b | 5 | **Bloom and fog were built and authored off.** ADR 0056 and ADR 0073 both shipped, and the game that exists to showcase the engine used neither — the exact "written, tested, exercised by zero content" pattern this gate exists to close, twice in one file. The Scarp's reason for bloom off (daylight with nothing above threshold) does not transfer: the Atrium has a **bright sky rectangle punched through a dark interior ceiling**, which is the textbook threshold-bloom case, and a 20 m interior with a shaft of sun in it is the textbook case for a little fog | `atrium.environment` authors both, and a capture shows the lamp blooming and depth reading across the room | ✅ **done** (s21) — bloom 0.3 at threshold 1.0, fog density 0.016 from 3 m in a cool haze |
