| 12c | — | **A sky for `games/atrium`.** See the box above: `sky ""` blocked four things at once, and the two existing `.hdr` generators are the precedent | The Atrium names a real environment map, and the lamp, the glass and the wall gradient each read as themselves in a capture | 🟡 **map built and the two jobs split (ADR 0079); blocked behind 12d** (s21) — review 9 asked for the ambient/backdrop split and it landed. But the sky is **not visible from anywhere the player can stand**, so nothing about it can be judged in play. See 12d |
| 12d | — | **The Atrium is 20 x 20 x 4 m and the camera sits at y = 3.69, thirty-one centimetres below the roof.** So every upward view is flat grey, the oculus cannot be seen from any standing position, and the best capture this project has produced was taken by editing the scene to move the camera. Found while verifying 12c with the new `--pitch` flag: at pitch 25 deg the view ray meets the roof underside at z = 5.0, *before* it reaches the opening at z = 4.0. A 20 m room 4 m high is a warehouse, not an atrium | A capture from the player's own camera, with no scene edit, shows the oculus and the light coming through it | ⬜ |
| 12e | — | **`amadeo capture --pitch/--yaw`** (ADR 0079's sibling; review 9 item 3). A capture could only ever show the authored view, and this project judges nearly everything by capture -- so checking a ceiling meant editing the game, capturing, and reverting, which was done repeatedly and twice left an edit behind | A reviewer can look anywhere in a game without modifying it, and the cameras are provably put back | ✅ **done** (s21) |# 13 — The engine gate
...
| 12c | — | **A sky for `games/atrium`.** See the box above: `sky ""` blocked four things at once, and the two existing `.hdr` generators are the precedent | The Atrium names a real environment map, and the lamp, the glass and the wall gradient each read as themselves in a capture | 🟡 **built, awaiting the critic** (s21) |
