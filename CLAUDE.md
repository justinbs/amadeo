...
🟡 amadeo-cli         the `amadeo` binary. Built: describe/query/entity/schedule/status/call/check/
                     replay/fmt/assets/**audio**/**anim**/import/import-gltf/snapshot/capture/**image**
                     — the last being `probe`/`row`/`col`/`crop`/`stats`/`diff`, standalone like
                     `fmt`, because this project judges nearly everything by capture and `docs/14` §3
                     requires a number behind every claim about a picture. `crop` magnifies without
                     filtering on purpose; `diff` reports a **bounding box** of what moved, which is
                     the operation that follows almost every experiment (import takes `--assets <dir>` to work on a
...
games/               actual games built with the engine
  ...
  warren           M3's exit gate, in progress: first person, **generated**, and a loop you can win
                   ...
                   **And it makes a noise** (ADR 0059-0061): six clips from `cargo run -p warren
                   ...
                   parented *is* its aim.
                   **Its palette is `docs/11` §5a's since session 22, and was another game's before
                   that**: `plaster`, `carpet`, `steel` and `timber` -- a Victorian interior authored
                   before the design document existed. Now `lining` (cast-iron rings, lead-white over
                   rust, bolted flanges), `screed`, `fittings` and `bulkhead`, generated by
                   `cargo run -p warren --bin finishes` on top of `amadeo-texture`. **Not `surfaces`:
                   two binaries of one name in a workspace collide in rustdoc and nothing else
                   notices** (docs/07).
                   **`cargo run -p warren --bin moment` writes `snapshots/playing.snapshot`**, and
                   `amadeo capture --from` photographs it. Until session 22 nobody could take a
                   picture of this game **in play** -- it boots to its title screen, a capture flag
                   cannot dismiss a menu, and the protocol has no mutating methods -- so every frame
                   any reviewer had ever seen of it was the main menu, while `docs/13` §3 defines
                   POLISHED as *a frame from a real game*. The gate's own condition was unreachable
                   for thirteen reviews and the thing that found it was somebody trying to satisfy it.
                   It has a **reticle** (`docs/11` §8's "usability failure at the core verb"): a dim
...
| `docs/13-the-engine-gate.md` | **The current work, and read §1b first.** The engine gate's binding plan — every item the critic ordered, with a falsifiable close condition and a status, plus the three measurements tracked across reviews. **§1b is session 26's re-scope**, instructed by Justin: thirty-four open rows became six, `games/atrium` and `games/scarp` are frozen, and the order beyond this gate is **finish `games/warren` → the editor → the first published game** (a survival game in the Project Zomboid line, isometric, and not a demo — `docs/05` M4b). `docs/05` is superseded where the two disagree. |
| `docs/14-the-critic.md` | You are about to send work to the critic, or you are about to write in a status column. Its standing brief: the evidence a review must gather, the seven things this repository has fooled a reviewer with, the capture and pixel-probe commands, and the rule that **only a review may write ✅**. Written for the critic and for Justin; everyone else reads §6 and stays out. |
| `docs/15-the-designer.md` | **The support agent's brief.** Added session 26. The designer owns player experience, story, worldbuilding, theming and UI — what a thing *means* to a player — and nothing else. §2 has the rules of engagement: **both agents are binding on the implementer**, they work independently, and where the critic disagrees with a designer decision **the critic wins**. Only Justin and the critic may decline or stop it. Quiet on `games/warren`, whose design is settled; its full job starts with `docs/05` M4b. |
